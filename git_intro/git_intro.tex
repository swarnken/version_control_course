...
\begin{exerciseframe}
\frametitle{Exercise: help, config, init (10 minutes)}
\begin{itemize}
...
\pause
\begin{itemize}
  \item What happens when you rename the directory where you called \hlcommand{git init}?
  \item What happens when you delete the \hlcommand{.git} subdirectory?
  \item What happens when you call \hlcommand{git init} a second time in a Git directory?
  \item Type \hlcommand{git config -{}-list} in your \texttt{\$HOME} directory!
...
\begin{exerciseframe}
\frametitle{Exercise: status, add, commit (10 minutes)}
\begin{itemize}
  \item Create files in your Git repository
  \item Add them to the staging area
  \item Make one or more commits
  \item Create two files, stage and commit only one of them.
  \begin{itemize}
    \item Modify the committed file
    \item Call \hlcommand{git status}\\
          (one file should be modified, the other one still untracked)
    \item Now: Run \hlcommand{git commit -a} without staging anything first!
    \item What happens? 
  \end{itemize}
\end{itemize}
\pause
\begin{itemize}
  ...
  \item Make some changes to one of your files and stage it.
  \begin{itemize}
    \item Now: Make some more changes
    \item Call \hlcommand{git status}
    \item What do you see?
    \item What version of the file is put into the repository when 
          you call \hlcommand{git commit} now without staging the file
          a second time?
  \end{itemize}
\end{itemize}
\end{exerciseframe}
...
\begin{exerciseframe}
\frametitle{Exercise: log, diff, show (5 minutes)}
\begin{itemize}
  \item Add some more files and make more commits.
  \item Use \hlcommand{git log} to view the history.
  \item Use \hlcommand{git log -p <path>} to see differences in history\\
        for one of your files.
  \item Make changes to one file and show the difference with git diff
  \item Add the file and use \hlcommand{git diff -{}-cached} to see what will be
        committed.
  \item Commit the change and and use the  \hlcommand{-v} switch to see what's to
        be committed in your editor.
  \item Use \hlcommand{git show} to inspect a version of the file from a previous
        commit.
\end{itemize}
\end{exerciseframe}
...
\begin{exerciseframe}
\frametitle{Exercise: checkout, reset (5 minutes)}
\begin{itemize}
\item Step 1: Create 5 files in your directory with\\
      one line of content in each file.
\item Step 2: Commit the files to the repository
\item Step 3: Change 2 of the 5 files and commit them.
...
\begin{exerciseframe}
\frametitle{Exercise: ignore, rename and remove (5 minutes)}
\begin{itemize}
  \item Add some files with extension \texttt{.log}. Do not stage them
  \item Add a \texttt{.gitignore} file to ignore these files.
  \item Git will not care about logfiles any more.
  \item The content of \texttt{.gitignore} may change over time.
  \item Why not put it into version control, too?\\
        $\rightarrow$ \hlcommand{git add .gitignore; git commit}
  \item Rename and remove some of your files.\\
        Do it with Git and without Git.
\end{itemize}
\end{exerciseframe}
